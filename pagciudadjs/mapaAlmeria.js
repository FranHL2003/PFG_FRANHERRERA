...
function loadMapa(ciudad,centro) {

    var map = L.map('mapa').setView(centro, 15);
    ...
    fetch("http://localhost/Proyecto%20fin%20de%20grado/test.php?province="+ciudad)
        .then(res => res.json())
        .then(data => {
            ...
            function loadMapa(ciudad) {
                // Obtener las coordenadas de la base de datos
                fetch("http://localhost/Proyecto%20fin%20de%20grado/pagciudad/obtener_coordenadas.php?ciudad=" + ciudad)
                  .then(res => res.json())
                  .then(data => {
                    if (data.length > 0) {
                      const centro = [data[0].x, data[0].y];
                      // Resto del código para cargar el mapa con las coordenadas obtenidas
                      var map = L.map('mapa').setView(centro, 15);
                      // ...
                    }
                  })
